...
After the node is created, the map creator can edit the node by performing a single click on the choice node. Then a modal opens where he can edit the title, the description and the already defined choices (Figure \ref{fig:gm-choicenode-mapcreator}). To delete a choice, the map creator only has to keep the textfields of the choice empty.
